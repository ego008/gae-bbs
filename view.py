...
from model import Comment, Counter, GoogleUser, KeyStrValue, Member, MemberInfo, Node, Topic
...
def read_img(img_url):
    ...
    res = urlfetch.fetch(url=img_url, headers=headers , method='GET')
                        ...
                        if nuser_obj.is_saved():
                            ...
                            try:
                                ...
                                if img_data:
                                    ...
                                    img_url = 'https://www.gravatar.com/avatar/%s?s=73&d=monsterid' % md5(req_user.email()).hexdigest()
                                    img_data = read_img(img_url)
                                    ...
                                    if avatar:
                                        ...
                                        if m_obj.flag ==1:
                                            if m_obj.id == 1:
                                                ...
                                                m_obj.flag = 2
                                            db.run_in_transaction(obj_runput,m_obj, ['cur_user:'+req_user.email()])
                            except:
                                logging.error('Fetch Google Plus Profile Picture Failure: '+ req_user.email())
                            #
