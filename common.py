...
def content_formate(text, is_spider=False):
    ...
    if 'www.tudou.com' in text:
        if 'programs/view' in text:
            ...
            text = _re_tudou.sub(r'<embed src="http://www.tudou.com/l/\2/&resourceId=0_05_05_99&iid=152278332&bid=05/v.swf" quality="high" width="590" height="492" align="middle" allowScriptAccess="sameDomain" type="application/x-shockwave-flash"></embed>', text)
...
def date_web(timestamp):
    it = int(timestamp)
    if (int(time())-it)<31536000:#365day 31536000
...
class BaseHandler(yui.HtmlRequestHandler):
    ...
    def echo(self, template, context=None, globals=None, layout=False):
        ...
        if self.request.user and not context['cur_user'] and self.request.path != '/setname':
            ...
            return
        if layout and self.__IOS_PATTERN.search(self.request.headers.get('user-agent','')):
            layout = 'ios_'+layout
            template = 'ios_'+template
            
        self.write(self.render(template, context, globals, layout))
    
    @yui.CachedProperty
    @memcached('cur_user', 300, lambda self: self.request.user.email() if self.request.user else '')
    def cur_user(self):
